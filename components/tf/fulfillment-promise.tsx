...
export function TfFulfillmentPromise({
  ...
  return (
    <section aria-label="Delivery and returns" className="rounded-xl border border-tf-stone-200 bg-tf-raised p-4 shadow-tf-sm">
      <h2 className="font-tf-display text-sm font-semibold text-tf-ink">Delivery and returns</h2>
      <div className={`mt-3 grid gap-3 ${compact ? "" : "sm:grid-cols-2"}`}>
        ...
        <div className="flex gap-2.5 text-sm"><RotateCcw aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-tf-primary" /><div><p className="font-medium text-tf-ink">Returns & exchanges</p><p className="text-sm leading-relaxed text-tf-stone-600">{returnPolicy?.trim() || "Returns policy not provided. Confirm eligibility, time limits and return delivery costs with the seller before paying."}</p></div></div>
      </div>
