...
// Price preset quick-filters
const PRICE_PRESETS = [
  { label: "Under R100", min: undefined, max: 100 },
              ...
              <div className="flex w-max items-center gap-2 py-2.5">
                ...
                {topCategories.map((c) => (
                  <button
                    key={c.slug}
                    type="button"
                    onClick={() => navigate({ category: currentFilters.category === c.slug ? undefined : c.slug })}
                    aria-pressed={currentFilters.category === c.slug}
                    className={cn(pillBase, currentFilters.category === c.slug ? pillActive : pillIdle)}
                  >
                    {c.name}
      ...
      <main className="mx-auto max-w-6xl px-4 pt-5 sm:px-6">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {PRICE_PRESETS.map((preset) => {
            const isActive = activePricePreset === preset;
            return (
              <button
                key={preset.label}
                type="button"
        ...
        {allProducts.length === 0 ? (
          <TfEmptyState
            icon={<SearchX />}
            title={activeCategoryName ? `No ${activeCategoryName} yet` : "No products match"}
            description={
              activeCategoryName && !currentFilters.search
                ? `Be the first to list a ${activeCategoryName} product on TradeFeed.`
            ...
            action={
              <div className="flex flex-wrap justify-center gap-2">
                {activeCategoryName && !currentFilters.search ? (
                  ...
                  >
                    List a {activeCategoryName} for free
                    <ArrowRight aria-hidden="true" className="size-3.5" />
                  </Link>
                ) : (
                  <TfButton
                    variant="secondary"
                    onClick={() => { setSearch(""); router.push("/marketplace"); }}
                  >
                    Clear everything
                  </TfButton>
                )}
