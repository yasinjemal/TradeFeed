...
// Shows all 12 nav items + shop name + catalog link.
//
// CRITICAL FIX: Previously, dashboard nav was desktop-only
// (hidden md:flex). On mobile, sellers had ZERO navigation.
// ============================================================
...
const navItems = [
  ...
  {
    label: "Referrals",
    href: (slug: string) => `/dashboard/${slug}/referrals`,
    exact: false,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  {
    label: "Team",
    href: (slug: string) => `/dashboard/${slug}/team`,
    exact: false,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[280px] bg-white shadow-2xl animate-in slide-in-from-left duration-300 flex flex-col">
            ...
            <div className="border-t border-slate-100 px-4 py-3 space-y-2">
              <Link
                ...
                target="_blank"
                onClick={handleClose}
                className="flex items-center gap-2 text-sm font-medium text-emerald-600 hover:text-emerald-700 px-3 py-2 rounded-xl hover:bg-emerald-50 transition-colors"
              >
