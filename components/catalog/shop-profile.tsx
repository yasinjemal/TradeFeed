...
export function ShopProfile({ shop, tierBadge }: ShopProfileProps) {
  ...
  const hasRichProfile = hasLocation || hasAddress || hasHours || hasSocials || hasAbout || hasGallery;
  if (!hasRichProfile) {
    // Minimal trust section for new sellers with no profile details yet
    const joinDate = shop.createdAt.toLocaleDateString("en-ZA", {
      month: "long",
      year: "numeric",
    });
    return (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-500">Member since {joinDate}</p>
            </div>
          </div>
          <a
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#25D366] text-white text-xs font-medium hover:bg-[#20bd5a] transition-colors"
          >
