// ============================================================
// Marketplace Activity — Live trust signals bar
// ============================================================
// Displays key activity stats to make the marketplace feel
// alive and trustworthy. Uses deterministic values seeded from
// totalProducts to avoid hydration mismatch.
// ============================================================
...
export function MarketplaceActivity({ totalProducts, totalShops }: MarketplaceActivityProps) {
  const t = useTranslations("marketplace.activity");
  ...
  return (
    <motion.div
      className="inline-flex items-center gap-3 sm:gap-5 flex-wrap justify-center px-5 py-2.5 rounded-2xl bg-white/60 backdrop-blur-sm border border-slate-200/60 shadow-sm"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: 0.3, ease: [0.22, 1, 0.36, 1] }}
    >
      <ActivityPill color="green" label={t("soldToday", { count: soldToday })}>
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
        </svg>
      </ActivityPill>
      <ActivityPill color="blue" label={t("browsingNow", { count: browsing })}>
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
        </svg>
      </ActivityPill>
      <ActivityPill color="emerald" label={t("sellersOnline", { count: sellersOnline })}>
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
