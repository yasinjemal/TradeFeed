    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-semibold text-stone-900 truncate">{shop.name}</h1>
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {shop.city && (
              <>
                ...
                <span className="text-xs text-stone-500">{shop.city}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <p className="text-xs text-stone-500 font-medium">Orders today</p>
          <p className="text-2xl font-semibold text-stone-900 mt-1">{stats.ordersToday}</p>
        </div>
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <p className="text-xs text-stone-500 font-medium">Revenue today</p>
          <p className="text-2xl font-semibold text-stone-900 mt-1">{formatZAR(stats.revenueTodayCents)}</p>
        </div>
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <p className="text-xs text-stone-500 font-medium">Products</p>
          <p className="text-2xl font-semibold text-stone-900 mt-1">{stats.productCount}</p>
        </div>
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <p className="text-xs text-stone-500 font-medium">Total revenue</p>
          <p className="text-2xl font-semibold text-stone-900 mt-1">{formatZAR(orderStats.revenueCents)}</p>
        </div>
      </div>
      ...
      {priorities.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-stone-500 mb-3">Needs your attention</h2>
          <div className="space-y-2">
            {priorities.map((item, i) => (
              <Link
                key={i}
                href={item.href}
                className="flex items-center gap-3 bg-white rounded-xl border border-stone-200 px-4 py-3 hover:border-stone-300 transition-colors group"
              >
                <span
                ...
                <svg className="w-4 h-4 text-stone-300 group-hover:text-stone-500 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-medium text-stone-500">Recent products</h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-sm font-medium text-stone-900">{formatZAR(minPrice)}</p>
                      <p className={`text-xs ${totalStock > 0 ? "text-stone-400" : "text-red-500 font-medium"}`}>
      ...
      {/* ── Quick Actions (2\u00D72) ────────────────────── */}
      <div>
        <h2 className="text-sm font-medium text-stone-500 mb-3">Quick actions</h2>
        <div className="grid grid-cols-2 gap-3">
          <Link
            href={`/dashboard/${slug}/products/new`}
            className="flex items-center gap-3 bg-white rounded-xl border border-stone-200 p-4 hover:border-emerald-300 hover:bg-emerald-50/30 transition-colors group"
          >
            ...
            <span className="text-sm font-medium text-stone-700">Add Product</span>
          </Link>
          <Link
            href={`/dashboard/${slug}/orders`}
            className="flex items-center gap-3 bg-white rounded-xl border border-stone-200 p-4 hover:border-stone-300 transition-colors group"
          >
            ...
            <span className="text-sm font-medium text-stone-700">Orders</span>
          </Link>
          <Link
      ...
      {/* ── Shop Health ────────────────────────────── */}
      <div>
        <h2 className="text-sm font-medium text-stone-500 mb-3">Shop health</h2>
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="text-2xl font-semibold text-stone-900">{health.score}</span>
          ...
          <div className="mt-4 space-y-2">
            {[
              ...
              <div key={d.label} className="flex items-center gap-3">
                <span className="text-xs text-stone-500 w-28 flex-shrink-0">{d.label}</span>
                ...
                <span className="text-xs text-stone-400 w-10 text-right">{d.value}/{d.max}</span>
              </div>
            ))}
          </div>
          {health.suggestions.length > 0 && (
            <div className="mt-3 pt-3 border-t border-stone-100">
              <p className="text-xs text-stone-500 mb-1.5">Suggestions</p>
