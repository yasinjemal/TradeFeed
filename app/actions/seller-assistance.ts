...
import {
  approveSellerAssistance,
  cancelSellerAssistance,
} from "@/lib/db/seller-assistance";
...
export async function approveSellerAssistanceAction(input: unknown) {
  try {
    ...
    const campaign = await approveSellerAssistance(
      admin.id,
      parsed.userId,
      parsed.shopId,
      parsed.fingerprint,
    ...
    await logAdminAction({
      ...
      action: "seller_assistance_approved",
    ...
    return {
      success: true,
      message:
        "Approved and queued. Delivery will recheck eligibility and sending settings.",
    };
  } catch {
    return {
      success: false,
      message:
        "Unable to approve. Refresh: the message may have changed, already been queued, or become ineligible.",
    };
  }
}
  ...
  try {
    ...
    if (!count)
      return {
        success: false,
        message:
          "This email has already been claimed or completed. Refresh its status.",
      };
    await logAdminAction({
      adminId: admin.clerkId,
      adminEmail: admin.email,
      action: "seller_assistance_cancelled",
      entityType: "email_campaign",
      entityId: campaignId,
    });
    revalidatePath("/admin/activation");
